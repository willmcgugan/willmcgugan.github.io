...
layout: post
title: Milkshake
categories: [tech,soure grapes,side mission,markdown]
---
...
You see until recently I was the CEO of Textualize, a startup to build rich applications for the terminal.
...
| [![Toad](../images/toad3.png)](../images/toad3.png) | [![Toad](../images/toad4.png)](../images/toad4.png) | [![Toad](../images/toad1.png)](../images/toad1.png) |
...
By "jank", I mean anything happening in the UI which is unpleasant or detracts from the user experience rather than enhances it.
...
If you resize the terminal, the output will be garbled if it doesn't happen to be in the last live portion the app is updating.
...
- **Python is slow**. Python has gotten a lot faster over the years. And while it doesn't have the performance of other languages, it is more than fast enough for a TUI. Textual runs just fine on a Raspberry Pi.
...
- **Python startup time is slow**. There is some truth in this, although we are only talking 100ms or so. And I don't see any practical difference between Python and JS apps on the terminal.
...
I was still reparsing the Markdown document.
Markdown-it-py is an excellent library and really quite fast, but a very large document could take a few milliseconds to parse. Multiply that by 100 tokens a second and it becomes significant.

The fix was essentially the same as optimization 1.
Since only the last block can change, I can store the line where it starts.
When new content arrives, I can start parsing from that point rather than the start of the document.
So no matter how large the document is, parsing will be sub 1ms.
...
They probably paid for the tokes, after all.
...
The end result is that the display is only a few milliseconds behind the data itself.
